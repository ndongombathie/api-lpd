...
class CaissierCaisseJournalController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return response()->json(
             [
                'liste_caisiers' => $journals,
                'somme_encaissements' => $journals->sum('total_encaissements'),
                'somme_decaissements' => $journals->sum('total_decaissements'),
                'somme_solde' => $journals->sum('solde_reel'),
                'nombre_de_caissier' => $journals->count(),
             ]);
    }
    ...
    public function show(string $date)
    {
        ...
        if (!$journal) {
            $fondOuverture = (int) $this->getFondOuverture(Carbon::parse($dateStr));
            [$totalEncaissements, $totalDecaissements, $soldeTheorique, $nombrePaiements] = $this->computeTotals($dateStr, $fondOuverture);
            return response()->json([
                'date' => $dateStr,
